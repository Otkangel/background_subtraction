...
unsigned char *remove_image(unsigned char *background, unsigned char *foreground, unsigned char *newground, int width, int height, int channel)
{
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            {
                threshold = abs(foreground[i * width * channel + j * channel + 4] - background[i * width * channel + j * channel + 4]);
                if ((51.5 <= threshold) )
                {
                    newground[i * width * channel + j * channel + k] = foreground[i * width * channel + j * channel + k];
                }
            } 
        }
    }
